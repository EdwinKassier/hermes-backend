...
class TestVectorDatabaseConnection:
    ...
    def test_embedding_generation(self, gemini_service):
        ...
        assert actual_dimensions in valid_dimensions, f"Unexpected embedding dimensions: {actual_dimensions}"
        assert all(isinstance(x, (int, float)) for x in embedding)
        logger.info(f"✓ Generated embedding with {actual_dimensions} dimensions (expected one of {valid_dimensions})")
...
class TestVectorSimilaritySearch:
    ...
    def test_direct_similarity_search(self, gemini_service):
        ...
        for doc, score in results:
            ...
            logger.info(f"  Result score: {score:.3f}, preview: {doc.page_content[:100]}")
    ...
    def test_similarity_threshold_filtering(self, gemini_service):
        ...
        logger.info(f"✓ Threshold filtering works: {len(high_threshold_results)} @ 0.8, {len(low_threshold_results)} @ 0.5")
    
    @pytest.mark.parametrize("query,expected_keywords", [
        ("Edwin Kassier education", ["university", "degree", "education", "study", "school"]),
        ("Edwin Kassier work experience", ["engineer", "software", "developer", "work", "experience"]),
        ("Edwin Kassier projects", ["project", "built", "created", "developed", "application"]),
        ("Edwin Kassier skills", ["programming", "python", "javascript", "development", "technology"]),
    ])
...
class TestRAGQuality:
    ...
    @pytest.mark.parametrize("query,expected_keywords", [
        ("Edwin Kassier skills", ["software", "engineering", "programming", "development"]),
    ...
    def test_rag_answer_quality(self, gemini_service, query, expected_keywords):
        ...
        if found:
            ...
            logger.warning(f"⚠ No expected keywords in response for '{query}': {response[:100]}")
    ...
    def test_rag_factual_accuracy(self, gemini_service):
        ...
        for fake_lang in fake_languages:
            assert fake_lang not in response.lower(), \
    ...
    def test_rag_returns_dont_know_when_appropriate(self, gemini_service):
        ...
        response_lower = response.lower()
        indicates_uncertainty = any(ind in response_lower for ind in uncertainty_indicators)

...
from app.prism.exceptions import BotCreationError, SessionNotFoundError
from app.prism.models import AudioChunkOutgoing, PrismSession
...
class TestSessionManagement:
    ...
    def test_close_session_idempotent(self, prism_service):
        ...
        prism_service.session_store.get_session.side_effect = SessionNotFoundError(
            "sess_123"
        )
...
class TestBotManagement:
    ...
    def test_handle_bot_state_change(self, prism_service):
        ...
        prism_service.handle_bot_state_change(
            bot_id="bot_789", state="joined_recording"
        )
    ...
    def test_map_attendee_state_to_bot_state(self, prism_service):
        ...
        assert (
            prism_service._map_attendee_state_to_bot_state("joining")
            == BotState.JOINING
        )
        assert (
            prism_service._map_attendee_state_to_bot_state("joined_recording")
            == BotState.IN_MEETING
        )
...
class TestTranscriptProcessing:
    ...
    def test_handle_transcript_skip_interim(self, prism_service):
        ...
        prism_service.handle_transcript(
            session_id="sess_123", speaker="John", text="Hello...", is_final=False
        )
    ...
    def test_should_skip_obvious_non_response(self, prism_service):
        ...
        assert (
            prism_service._should_skip_obvious_non_response("What time is it?") is False
        )

        # Bot mention - should NOT skip
        assert prism_service._should_skip_obvious_non_response("Hey Prism") is False
        assert (
            prism_service._should_skip_obvious_non_response("Can the bot help?")
            is False
        )
    ...
    def test_get_pending_audio(self, prism_service):
        ...
        for i in range(3):
            mock_session.audio_queue.append(
                AudioChunkOutgoing(
                    data=b"chunk_data", timestamp=datetime.utcnow(), sequence=i
                )
            )

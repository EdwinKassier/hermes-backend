...
class ChatterboxRequestQueue:
    ...
    def __init__(self, max_queue_size: int = 10):
        ...
        logger.info(
            f"Chatterbox request queue initialized (max size: {max_queue_size})"
        )

    def enqueue(self, func: Callable, *args, timeout: float = 300.0, **kwargs) -> Any:
        """
...
class ChatterboxTTSProvider(BaseTTSProvider):
    ...
    def __init__(self, device: Optional[str] = None, cloud_storage_service=None):
        """
    ...
    def _generate_audio_impl(
        ...
        try:
            self.model.prepare_conditionals(str(audio_path), exaggeration=exaggeration)
        except Exception as e:
            raise RuntimeError(
                f"[Chatterbox TTS] Failed to prepare conditionals: {e}"
            ) from e
        ...
        try:
            ...
            audio_data = self.model.generate(
                cleaned_text, temperature=temperature, cfg_weight=cfg_weight
            )

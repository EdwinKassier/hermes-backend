...
class RedisSessionStore:
    ...
    def get_session(self, session_id: str) -> Optional[PrismSession]:
        ...
        try:
            ...
            if (
                "transcript_history" in session_dict
                and session_dict["transcript_history"]
            ):
                for transcript in session_dict["transcript_history"]:
                    if "timestamp" in transcript and isinstance(
                        transcript["timestamp"], str
                    ):
                        transcript["timestamp"] = datetime.fromisoformat(

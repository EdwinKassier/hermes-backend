...
from .constants import ATTENDEE_API_BASE_URL, ATTENDEE_API_VERSION, AUDIO_SAMPLE_RATE
...
class AttendeeClient:
    ...
    def create_bot(
        ...
        payload = {
            ...
            "websocket_settings": {
                "audio": {"url": audio_websocket_url, "sample_rate": AUDIO_SAMPLE_RATE}
            },
        }
        ...
        try:
            ...
            if response.status_code not in (200, 201):
                ...
                logger.error(
                    f"Bot creation failed (HTTP {response.status_code}): {error_msg}"
                )
                logger.error(
            ...
            if not bot_id:
                ...
                raise BotCreationError(
                    f"Invalid API response: missing bot ID. Response keys: {list(data.keys())}"
                )
    ...
    def unmute_bot(self, bot_id: str) -> bool:
        ...
        try:
            ...
            if response.status_code not in (200, 201):
                error_msg = self._parse_error(response)
                logger.error(
                    f"Bot unmute failed (HTTP {response.status_code}): {error_msg}"
                )
    ...
    def send_chat_message(self, bot_id: str, message: str) -> bool:
        ...
        try:
            ...
            if response.status_code not in (200, 201):
                error_msg = self._parse_error(response)
                logger.error(
                    f"Chat message failed (HTTP {response.status_code}): {error_msg}"
                )
                raise AttendeeAPIError(error_msg)
    ...
    def send_output_audio(
        self, bot_id: str, audio_data: bytes, content_type: str = "audio/wav"
    ) -> bool:
        """
        ...
        try:
            logger.info(
                f"Sending {len(audio_data)} bytes of audio to bot {bot_id} via output_audio endpoint"
            )
            ...
            payload = {"data": audio_base64, "type": "audio/mp3"}

            response = self.session.post(
                endpoint, json=payload, timeout=30  # Longer timeout for audio upload
            )

            if response.status_code not in (200, 201, 202):
                error_msg = self._parse_error(response)
                logger.error(
                    f"Audio output failed (HTTP {response.status_code}): {error_msg}"
                )
                raise AttendeeAPIError(error_msg)

            logger.info(
                f"✅ Audio sent successfully to bot {bot_id} ({len(audio_data)} bytes)"
            )
            return True

...
from google.cloud import texttospeech # Import here to keep dependency local

if TYPE_CHECKING:
    from google.cloud import texttospeech as GcpTextToSpeechModule
...
@contextmanager
def _patched_torch_load_context(map_location_device: torch.device) -> None:
    ...
    _original_torch_load_ref = torch.load # Use the module-level _original_torch_load

    def patched_load(*args, **kwargs):
        if 'map_location' not in kwargs:
            kwargs['map_location'] = map_location_device
        return _original_torch_load_ref(*args, **kwargs)
    ...
    try:
        ...
        torch.load = _original_torch_load_ref # Restore to the original captured one
...
class TTSService:
    ...
    def _load_chatterbox_model(self) -> None:
        ...
        try:
            with _patched_torch_load_context(self.map_location):
        ...
        try:
            ...
            if credentials_path:
                ...
                logger.warning(
                    ...
                    "and no credentials_path provided. Google TTS might fail to authenticate."
                )
            ...
            logger.error(
                ...
                "Please install it to use the Google TTS provider (e.g., pip install google-cloud-texttospeech)."
            )
    ...
    def _generate_audio_chatterbox(
        ...
        try:
            logger.info(
                ...
                text_input[:50], temperature_input, cfgw_input, seed_num_input
            )
            audio_data = self.tts_model.generate(
                text_input, temperature=temperature_input, cfg_weight=cfgw_input
            )
            ta.save(actual_output_filepath, audio_data, self.sample_rate) # self.sample_rate is from Chatterbox model
    ...
    def _generate_audio_google(
        ...
        logger.info(
            ...
            text_input[:50], final_voice_params, final_audio_config["audio_encoding"], current_sample_rate
        )
    ...
    def generate_audio(
        ...
        audio_prompt_path_input: str = "TrainingShort.wav", # Default for Chatterbox
        ...
        if self.tts_provider == "chatterbox":
            ...
            chatterbox_params_provided = any([
                audio_prompt_path_input != "TrainingShort.wav", # Check if different from default

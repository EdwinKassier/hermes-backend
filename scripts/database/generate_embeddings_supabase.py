...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("embedding_generation.log")],
)
...
class EmbeddingConfig:
    ...
    GEMINI_VISION_MODEL = (
        "gemini-2.5-flash"  # Using the specified multimodal model for OCR
    )
...
def embed_texts_with_gemini(
    ...
    try:
        ...
        for i in range(0, len(texts), config.BATCH_SIZE):
            ...
            logger.info(
                f"Generated embeddings for {min(i + len(batch), len(texts))}/{len(texts)} chunks."
            )
...
def extract_text_with_gemini(
    ...
    try:
        ...
        logger.info(
            f"Successfully extracted text from '{os.path.basename(file_path)}' using Gemini."
        )
        return full_text
    except Exception as e:
...
def process_document(
    file_path: str, config: EmbeddingConfig
    ...
    try:
        ...
        if len(cleaned_content) < config.MIN_TEXT_LENGTH:
            logger.warning(
                f"Skipping '{file_name}' due to insufficient content after cleaning."
            )
            return None
        ...
        metadata = {
            ...
            "ocr_model": (
                config.GEMINI_VISION_MODEL if mime_type == "application/pdf" else "N/A"
            ),
            "title": (
                extract_title(cleaned_content)
                if config.ENABLE_TITLE_EXTRACTION
                else "N/A"
            ),
            "author": "Unknown",
...
def load_documents_from_gcs(
    bucket_name: str, folder_path: str, config: EmbeddingConfig
) -> List[Dict]:
...
def process_and_split_documents(
    docs: List[Document], splitter: EnhancedDocumentSplitter
) -> List[Document]:
    return [chunk for doc in docs for chunk in splitter.split_document(doc)]


def process_batch(
    batch: List[Document], supabase: SupabaseClient, config: EmbeddingConfig
) -> None:
    try:
    ...
    try:
        with psycopg2.connect(db_url) as conn, conn.cursor() as cur:
            ...
            """
                    ...
                    sql.Literal(config.HNSW_M), sql.Literal(config.HNSW_EF_CONSTRUCTION)
                )
            )
            ...
            """
                    ...
                    sql.Literal(config.EMBEDDING_DIMENSIONS),
                    sql.Literal(config.EMBEDDING_DIMENSIONS),
                )
            )

        logger.info(
            "Supabase table, indexes, and match_documents function are configured correctly."
        )
    except Exception as e:
...
def main():
    ...
    try:
        ...
        logger.info(
            f"Using Gemini API embedding model '{config.EMBEDDING_MODEL}' with {config.EMBEDDING_DIMENSIONS} dimensions."
        )

        # Initialize Vertex AI for OCR (Gemini Vision)
        logger.info(
            f"Initializing Vertex AI for OCR: project '{env['google_project_id']}', location '{env['google_project_location']}'..."
        )
        vertexai.init(
            project=env["google_project_id"], location=env["google_project_location"]
        )
